...
(function dartProgram() {
  ...
  D = {
    ...
    Sprite$: function(isShip, size, rng) {
      ...
      t5 = new T.Vector2(t4);
      t5.setValues$2(t2, t3);
      t5.sub$1(0, $.$get$middle());
      t4[1] = t4[1] * 2;
      t4[0] = t4[0] * 2;
      t4 = rng.nextDouble$0();
      t3 = 6.283185307179586 * rng.nextDouble$0();
      t2 = rng.nextDouble$0();
      t6 = rng.nextDouble$0();
      t7 = rng.nextDouble$0();
      ...
      t8[1] = t6;
      t8[2] = t7;
      t1.setValues$2(t4 * Math.sin(t3), t4 * Math.cos(t3));
      return new D.Sprite(isShip, size, t5, t3, t4, t1, new T.Vector3(t8));
    },
    ExtractSpritePositions: function(sprites, out) {
      var t1, t2, offset, _i, s, t3, t4, offset0, t5, dx, dy, d2x, d2y, t6;
      H.assertSubtype(sprites, "$isList", [D.Sprite], "$asList");
      for (t1 = sprites.length, t2 = out.length, offset = 0, _i = 0; _i < sprites.length; sprites.length === t1 || (0, H.throwConcurrentModificationError)(sprites), ++_i) {
        s = sprites[_i];
        t3 = offset + 1;
        t4 = offset + 2;
        offset0 = offset + 3;
        ...
        if (s.isShip) {
          ...
          t6 = t5[0];
          if (offset >= t2)
            return H.ioore(out, offset);
          out[offset] = t6 + dx * 0.07;
          t6 = t5[1];
          if (t3 >= t2)
    ...
    ExtractSpriteColors: function(sprites, out) {
      var t1, t2, offset, _i, s, t3, t4, t5, i;
      H.assertSubtype(sprites, "$isList", [D.Sprite], "$asList");
      for (t1 = sprites.length, t2 = out.length, offset = 0, _i = 0; _i < t1; ++_i) {
        s = sprites[_i];
        t3 = s.color._v3storage;
        t4 = t3[0];
    ...
    ExtractSpriteSizes: function(sprites, out) {
      var t1, t2, offset, _i, s, t3, i;
      H.assertSubtype(sprites, "$isList", [D.Sprite], "$asList");
      for (t1 = sprites.length, t2 = out.length, offset = 0, _i = 0; _i < t1; ++_i) {
        s = sprites[_i];
        t3 = s.size;
    ...
    HandleUseInput: function(input, gamepad, sprites) {
      var t1, _i, s, ship, otherStorage, t2, t3, t4, t5;
      H.assertSubtype(sprites, "$isList", [D.Sprite], "$asList");
      gamepad.Poll$0();
      t1 = input._currentlyPressedKeys;
      if (t1.contains$1(0, 37) || gamepad.x < -0.5) {
        if (0 >= sprites.length)
          return H.ioore(sprites, 0);
        t1 = sprites[0];
        t1.setAngle$1(t1.angle - 0.031415926535897934);
      } else if (t1.contains$1(0, 39) || gamepad.x > 0.5) {
        if (0 >= sprites.length)
          return H.ioore(sprites, 0);
        t1 = sprites[0];
        t1.setAngle$1(t1.angle + 0.031415926535897934);
      } else if (t1.contains$1(0, 38) || gamepad.y < -0.5) {
        if (0 >= sprites.length)
          return H.ioore(sprites, 0);
        t1 = sprites[0];
        t1.setVelocity$1(t1.velocity + 0.05);
      } else if (t1.contains$1(0, 40) || gamepad.y > 0.5) {
        if (0 >= sprites.length)
          return H.ioore(sprites, 0);
        t1 = sprites[0];
        t1.setVelocity$1(t1.velocity - 0.05);
      } else if (input._justPressedKeys.contains$1(0, 32) || gamepad.justPressed) {
        P.print("fire");
        for (t1 = sprites.length, _i = 0; _i < sprites.length; sprites.length === t1 || (0, H.throwConcurrentModificationError)(sprites), ++_i) {
          s = sprites[_i];
          ship = sprites[0];
          if (s.isInvisible$0()) {
            otherStorage = ship.pos._v2storage;
            t1 = s.pos._v2storage;
            t1[1] = otherStorage[1];
            t1[0] = otherStorage[0];
            t1 = ship.angle;
      ...
      gamepad.justPressed = false;
      input._justReleasedKeys.clear$0(0);
      input._justPressedKeys.clear$0(0);
    },
    main: function() {
      var _box_0, canvas, w, h, d, input, sprites, i, cgl, t1, t2, t3, t4, prog, points, sizes, colors, t5, t6, mesh, desc, index;
      _box_0 = {};
      canvas = H.interceptedTypeCheck(document.querySelector("#webgl-canvas"), "$isCanvasElement");
      w = canvas.clientWidth;
      h = canvas.clientHeight;
      d = Math.min(H.checkNum(w), H.checkNum(h));
      canvas.width = d;
      canvas.height = d;
      input = D.Keyboard$(null);
      sprites = H.setRuntimeTypeInfo([D.Sprite$(true, 50, C.C__JSRandom)], [D.Sprite]);
      for (i = 0; i < 10; ++i)
        C.JSArray_methods.add$1(sprites, D.Sprite$(false, 20, C.C__JSRandom));
      cgl = new G.ChronosGL(canvas);
      t1 = P.String;
      t2 = P.Object;
      t3 = (canvas && C.CanvasElement_methods).getContext$2(canvas, "webgl2", P.LinkedHashMap_LinkedHashMap$_literal(["alpha", false, "depth", true, "stencil", true, "antialias", false, "premultipliedAlpha", true, "preserveDrawingBuffer", false, "failIfMajorPerformanceCaveat", false], t1, t2));
      cgl._gl = t3;
      if (t3 == null)
      ...
      prog = G.RenderProgram$("basic", cgl, $.$get$demoVertexShader(), $.$get$demoFragmentShader());
      t3 = sprites.length;
      points = new Float32Array(3 * (t3 + 2));
      t3 = sprites.length;
      sizes = new Float32Array(t3 + 2);
      t3 = sprites.length;
      colors = new Float32Array(3 * (t3 + 2));
      D.ExtractSpritePositions(sprites, points);
      D.ExtractSpriteSizes(sprites, sizes);
      D.ExtractSpriteColors(sprites, colors);
      t3 = prog._cgl;
      t4 = prog._shaderObjectV._canonicalToLayoutPos;
      t5 = P.LinkedHashMap_LinkedHashMap$_empty(t1, t2);
      t6 = J.createVertexArray$0$x(t3._gl);
      mesh = new G.MeshData(t3, t6, 0, t5, t4, -1, P.LinkedHashMap_LinkedHashMap$_empty(t1, P.Float32List), "meshdata:points");
      t5.$indexSet(0, "aPosition", J.createBuffer$0$x(t3._gl));
      mesh.ChangeVertices$1(points);
      ...
      t3.vertexAttribPointer$7(0, t5.$index(0, "aPosition"), index, desc.GetSize$0(), 5126, false, 0, 0);
      mesh.AddAttribute$3("aPointSize", sizes, 1);
      mesh.AddAttribute$3("aColor", colors, 3);
      ...
      t1.$indexSet(0, "cBlendEquation", $.$get$BlendEquationAdd());
      t1.$indexSet(0, "cDepthTest", false);
      t1.$indexSet(0, "cDepthWrite", false);
      _box_0._lastTimeMs = 0;
      new D.main_animate(_box_0, sprites, input, new D.Gamepad(-1, 0, 0), points, mesh, prog, new G.Material(t1, "nodepth")).call$1(0);
    },
    Sprite: function Sprite(t0, t1, t2, t3, t4, t5, t6) {
      var _ = this;
      _.isShip = t0;
      _.size = t1;
      _.pos = t2;
    ...
    Gamepad: function Gamepad(t0, t1, t2) {
      var _ = this;
      _._game$_index = t0;
      _._gp = null;
      _.x = t1;
      _.y = t2;
      _.justPressed = _.button = false;
    },
  ...
  W.Gamepad0.prototype = {$isGamepad0: 1};
  W.History.prototype = {
    get$length: function(receiver) {
      return receiver.length;
    }
  ...
  W.Navigator.prototype = {
    getGamepads$0: function(receiver) {
      var gamepadList = receiver.getGamepads();
      if (gamepadList.prototype == null)
  ...
  W._GamepadList.prototype = {
    ...
    $indexSet: function(receiver, index, value) {
      H.interceptedTypeCheck(value, "$isGamepad0");
      throw H.wrapException(P.UnsupportedError$("Cannot assign element of immutable List."));
    },
    elementAt$1: function(receiver, index) {
      if (index < 0 || index >= receiver.length)
  ...
  G.MeshData.prototype = {
    ...
    AddAttribute$3: function(canonical, data, width) {
      ...
      instanced = C.JSString_methods._codeUnitAt$1(canonical, 0) === 105;
          ...
          case 117:
            if (t4.containsKey$1(0, t7)) {
              ...
              switch (t7) {
                ...
                case "vec3":
                  if (desc.arraySize === 0) {
                    t7 = C.JSNull_methods.get$storage(H.interceptedTypeCast(val, "$isVector3"));
          ...
          case 99:
            ...
            switch (t7) {
              case "cDepthTest":
                t7 = t3._gl;
                if (val === true)
                  J.enable$1$x(t7, 2929);
                else
                  J.disable$1$x(t7, 2929);
                break;
              case "cStencilFunc":
  ...
  T.Vector2.prototype = {
    ...
    $add: function(_, other) {
      ...
      t2 = new T.Vector2(t1);
      t2.setFrom$1(this);
      argStorage = other.get$_v2storage();
  ...
  T.Vector3.prototype = {
    ...
    $add: function(_, other) {
      ...
      t2 = new T.Vector3(t1);
      t2.setFrom$1(this);
      argStorage = other.get$_v3storage();
      t1[0] = C.JSNumber_methods.$add(t1[0], argStorage.$index(0, 0));
      t1[1] = C.JSNumber_methods.$add(t1[1], argStorage.$index(0, 1));
  ...
  D.Sprite.prototype = {
    setAngle$1: function(a) {
      this.angle = a;
      this.dir.setValues$2(this.velocity * Math.sin(a), this.velocity * Math.cos(this.angle));
    },
    setVelocity$1: function(v) {
      ...
      if (v < 0)
        v = 0;
      this.velocity = v;
      this.dir.setValues$2(v * Math.sin(this.angle), this.velocity * Math.cos(this.angle));
    },
    isInvisible$0: function() {
      var t1, t2;
      if (this.isShip)
        return false;
      t1 = this.pos._v2storage;
      t2 = t1[0];
      if (!(t2 > 1))
        if (!(t2 < -1)) {
          t1 = t1[1];
  ...
  D.Gamepad.prototype = {
    _LazyInitialize$0: function() {
      var t1, pads, t2, t3, count, _i, p;
      if (this._gp != null)
        return true;
      t1 = window.navigator;
      pads = (t1 && C.Navigator_methods).getGamepads$0(t1);
      for (t1 = pads.length, t2 = this._game$_index, t3 = t2 === -1, count = 0, _i = 0; _i < t1; ++_i) {
        p = pads[_i];
        if ((count === t2 || t3) && p != null) {
          H.printString("found gamepad: " + H.S(p.id) + " " + H.S(p.mapping));
          H.printString("buttons: " + p.buttons.length + " axes: " + J.get$length$asx(p.axes));
          this._gp = p;
          this._game$_index = count;
          return true;
        }
        ++count;
      }
    ...
    Poll$0: function() {
      var t1, t2, bs, _i, t3;
      if (!this._LazyInitialize$0()) {
        t1 = $.$get$info();
        (t1 && C.DivElement_methods).setInnerHtml$1(t1, "no joystick detected");
        return;
      }
      t1 = window.navigator;
      t1 = (t1 && C.Navigator_methods).getGamepads$0(t1);
      t2 = this._game$_index;
      if (t2 < 0 || t2 >= t1.length)
        return H.ioore(t1, t2);
      this._gp = t1[t2];
      bs = H.setRuntimeTypeInfo([], [P.bool]);
      for (t1 = this._gp.buttons, t2 = t1.length, _i = 0; _i < t1.length; t1.length === t2 || (0, H.throwConcurrentModificationError)(t1), ++_i)
        C.JSArray_methods.add$1(bs, t1[_i].pressed);
      t1 = $.$get$info();
      (t1 && C.DivElement_methods).setInnerHtml$1(t1, "device[" + this._game$_index + "]: " + H.S(this._gp.id) + "<br>axes: " + H.S(this._gp.axes) + "<br>buttons: " + H.S(bs));
      this.x = H.doubleTypeCheck(J.$add$ansx(J.$index$asx(this._gp.axes, 0), 0));
      this.y = H.doubleTypeCheck(J.$add$ansx(J.$index$asx(this._gp.axes, 1), 0));
      t1 = this.button;
      t2 = this._gp;
      t3 = t2.buttons;
      if (0 >= t3.length)
      ...
      if (0 >= t1.length)
        return H.ioore(t1, 0);
      this.button = t1[0].pressed;
    }
  };
  D.main_animate.prototype = {
    call$1: function(timeMs) {
      var t1, t2, t3, _i, s, argStorage, t4, t5;
      ...
      for (t1 = this.sprites, t3 = t1.length, t2 = (timeMs - t2) / 10000, _i = 0; _i < t3; ++_i) {
        s = t1[_i];
        argStorage = s.dir._v2storage;
        t4 = s.pos._v2storage;
        t4[0] = t4[0] + argStorage[0] * t2;
        t4[1] = t4[1] + argStorage[1] * t2;
        if (s.isShip) {
          t5 = t4[0];
          if (t5 < -1)
            t4[0] = 2 - t5;
      ...
      D.HandleUseInput(this.input, this.gamepad, t1);
      t2 = this.points;
      D.ExtractSpritePositions(t1, t2);
      t1 = this.mesh;
      t1.ChangeVertices$1(t2);
      this.prog.Draw$2(t1, H.setRuntimeTypeInfo([this.mat], [G.UniformGroup]));
...
;
  ...
  (function lazyInitializers() {
    ...
    _lazy($, "BlendEquationAdd", "$get$BlendEquationAdd", function() {
      return G.TheBlendEquation$(32774, 1, 1);
    });
    ...
    _lazy($, "demoVertexShader", "$get$demoVertexShader", function() {
      var t1, t2;
      t1 = G.ShaderObject$("demoVertexShader");
      t2 = [P.String];
      t1.AddAttributeVars$1(H.setRuntimeTypeInfo(["aPosition", "aPointSize", "aColor"], t2));
